...
# =============================================================================
# 5. PARSEO DE PLANILLA MAESTRA (CORREGIDO - DESTINO POR CODIGO + MOTRIZ 2)
# =============================================================================
def parsear_planilla_maestra(data, fname):
    """
    CORRECCIONES APLICADAS:
    1. Deteccion de destino por codigo explicito EN CUALQUIER COLUMNA (LI, SA, EB, PU)
    2. Deteccion de dobles por columna Motriz 2 > 0
    3. Servicios sin codigo -> PU-LI / LI-PU segun via
    4. Soporte para formato EFE real con columnas Motriz 1 y Motriz 2
    ...
    try:
        ...
        for sheet_name, df in dfs.items():
            ...
            if header_idx != -1:
                ...
                # NUEVO: Detectar columnas de Motriz 1 y Motriz 2
                ...
                # Construir pares de columnas con rangos ampliados
                pairs = []
                ...
                for i in range(header_idx + 1, len(df)):
                    ...
                    # NUEVO: Detectar Motriz 2 para servicios dobles
                    es_doble = False
                    if motriz2_cols:
                        for mc2 in motriz2_cols:
                            ...
                            if val_m2 and val_m2 not in ('0', '0.0', '', 'nan', 'none', 'None', 'NAN'):
                                try:
                                    ...
                                    # Si no es numero pero no esta vacio, podría ser un ID de motriz
                    ...
                    for col_viaje, col_srv, col_hora, col_config in pairs:
                        ...
                        # Extraer numero de servicio y posible codigo de destino
                        servicio_num = None
                        destino_cod = None
                        
                        # Buscar patron "501-LI", "501 LI", "501LI", "501/LI"
                        ...
                        # Si no se detecto doble por Motriz 2, buscar en columna config
                        ...
                        # Prioridad 1: Codigo explicito (LI, SA, EB, PU)
                        # Prioridad 2: Rango del numero de servicio
                        # Prioridad 3: Default PU-LI / LI-PU
                        # ======================================================
                ...
                for i in range(len(df)):
                    ...
                    # Detectar doble por Motriz 2 en modo no estructurado
                    es_doble = False
                    ...
                    for c_idx, val in enumerate(row_vals):
                        ...
                        if re.match(r'^\d{1,2}:\d{2}(:\d{2})?$', val):
                            ...
                            # Detectar doble por config si no se detecto por Motriz 2
                            if not es_doble:
