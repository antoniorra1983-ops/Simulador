...
from config import *
from etl_parser import mins_to_time_str, get_pax_at_km
...
def render_gemelo_digital(df_dia, df_dia_e, active_sers, fecha_sel, pct_trac, use_rm, use_pend, estacion_anio, prefix_key, gap_vias, pax_dia_total=0, df_vacios_real=None, km_limache_manual=0.0):
    """
    Gemelo Digital de la operación MERVAL.
    Se han eliminado todas las referencias a maniobras en vacío.
    """
    ...
    # Acumuladores de energía (solo trenes comerciales)
    energy_by_fleet = {'XT-100': 0.0, 'XT-M': 0.0, 'SFE': 0.0}
    ser_accum_visual = {s[1]: 0.0 for s in active_sers}
    reostato_injection = {s[1]: 0.0 for s in active_sers}
    ...
    for s_name in ser_accum_visual.keys():
        ...
        ser_neto_visual[s_name] = max(0.0, neto)
    ...
    km_total_red = df_inic['tren_km'].sum() if not df_inic.empty else 0.0
    ide_ac  = round(seat_accum_1 / max(1, km_total_red), 3) if km_total_red > 0 else 0.0
    ...
    if not df_inic.empty:
        ...
        st.markdown(f"<div style='background-color:#FFFDE7; border-radius:8px; padding:15px; text-align:center; border: 1px solid #FFF59D;'><div style='font-size:16px; font-weight:bold; color:#F57F17;'>SEAT EL SOL (Total Red + Pérdidas AC)</div><div style='font-size:26px; font-weight:bold; color:#F57F17; margin:10px 0;'>{seat_accum_1:,.0f} kWh</div><div style='font-size:13px; color:#666;'>Km Comercial: {df_inic['tren_km'].sum():,.1f} km</div><div style='font-size:14px; color:#333; font-weight:bold; margin-top:4px;'>Km Total Red: {km_total_red:,.3f} km</div><div style='font-size:16px; color:#C62828; font-weight:bold; margin-top:6px;'>IDE Global Real: {ide_ac:.3f} kWh/km</div></div>", unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
