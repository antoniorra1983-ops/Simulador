...
# =============================================================================
# 1. UTILIDADES DE TIEMPO Y FECHA
# =============================================================================
...
def extraer_fecha_segura(df_raw, fname):
    """Detección forense de fecha operativa en nombre de archivo o celdas."""
    ...
    # FIX: Extraer grupos de dígitos consecutivos de forma segura (Ej: "via1 010126" no debe mezclarse)
...
def _col_to_est_idx(col):
    estaciones = getattr(config, 'ESTACIONES', [])
    if not estaciones:
...
def procesar_thdr(data, fname, via_param=1):
    km_acum = getattr(config, 'KM_ACUM', [0.0, 0.7, 1.4, 2.2, 3.9, 6.0, 7.4, 8.3, 9.2, 10.2, 11.7, 19.1, 21.4, 23.3, 25.3, 26.4, 27.6, 28.5, 29.1, 30.4, 43.13])
    ec = getattr(config, 'EC', ['PU','BE','FR','BA','PO','RE','MI','VM','HO','CH','ES','VAL','QU','SO','EB','AM','CO','VL','SA','PE','LI'])
    ...
    try:
        ...
        # FIX: Búsqueda Exhaustiva y Robusta de Cabeceras
        header_idx = 1
        for i in range(min(20, len(raw))):
            row_vals = [str(x).upper() for x in raw.iloc[i].values if pd.notna(x)]
            row_str = ' '.join(row_vals)
            # Busca la fila de sub-cabecera repetitiva que distingue al THDR del título
        ...
        if not est_cols: return pd.DataFrame(), "No se detectaron columnas de estaciones con tiempos válidos."
...
def cargar_pax(data, fname, via_param=1):
    ...
    try:
        if fname.lower().endswith('.csv'):
            full = pd.read_csv(BytesIO(data), dtype=str) 
        else:
            eng = "openpyxl" if fname.lower().endswith(".xlsx") else "xlrd"
            full = pd.read_excel(BytesIO(data), dtype=str, engine=eng)
        ...
        df = pd.DataFrame({col_name: full.iloc[header_idx + 1:, c_idx].values for c_idx, col_name in col_mapping.items() if c_idx < full.shape[1]})
        ...
        for c in pax_cols + ['CargaMax']: 
            if c in df.columns: df[c] = df[c].apply(lambda x: int(re.sub(r'[^\d]', '', str(x).replace('.', '')) or 0))
        return df
